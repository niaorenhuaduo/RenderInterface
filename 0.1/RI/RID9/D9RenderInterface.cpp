...
const float GUIORD_TOPFRONT	= 0.0f;
const float GUIORD_FRONT		= 0.25f;
const float GUIORD_MID			= 0.5f;
const float GUIORD_BACK		= 0.75f;
const float GUIORD_BACKGROUND	= 1.0f;
...
HRESULT D9RenderInterface::VOnInit(const HWND hWnd) {
	...
	if(FAILED(mp2DRenderer->OnRestore(mpDevice, mpTexManager)))
		return E_FAIL;
...
HRESULT D9RenderInterface::OnLostDevice() {
	...
	if(FAILED(mpTexManager->OnLostDevice()))
		return E_FAIL;
...
HRESULT D9RenderInterface::OnRestore() {
	...
	if(FAILED(mpTexManager->OnRestore(mpDevice))) 
		return E_FAIL;
...
HRESULT D9RenderInterface::VOnResize(U32 Width, U32 Height, bool bWindowed) {
	...
	Log(" VOnResize: New Resolutin: %dx%d\n", Width, Height);
...
void D9RenderInterface::VDrawPic(const RIVec4 &topleft, const RIVec4 &topright,
	...
	float z = 0.0f;
	switch(zord) {
		case RIGUIORD_TOPFRONT:		z = GUIORD_TOPFRONT; break;
		case RIGUIORD_FRONT:		z = GUIORD_FRONT; break;
		case RIGUIORD_MID:			z = GUIORD_MID; break;
		case RIGUIORD_BACK:			z = GUIORD_BACK; break;
		case RIGUIORD_BACKGROUND:	z = GUIORD_BACKGROUND; break;
	}
