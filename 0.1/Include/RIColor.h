...
public:
	RIColor() { dwColor = 0; }
	RIColor(const RIColor &rhs) : dwColor(rhs.dwColor) {}
	RIColor& operator=(const RIColor &rhs) { a = rhs.a; r = rhs.r; g = rhs.g; b = rhs.b; return *this;}

	RIColor(float fC) { a = r = g = b = static_cast<U8>(fC*255.0f); }

	RIColor(float fA, float fR, float fG, float fB) { a = static_cast<U8>(fA*255.0f);
													  r = static_cast<U8>(fR*255.0f);
													  g = static_cast<U8>(fG*255.0f);
													  b = static_cast<U8>(fB*255.0f); }
	RIColor(float fR, float fG, float fB) { a = 255;
											r = static_cast<U8>(fR*255.0f);
											g = static_cast<U8>(fG*255.0f);
											b = static_cast<U8>(fB*255.0f); }

	RIColor(U8 _a, U8 _r, U8 _g, U8 _b) { a = _a; r = _r; g = _g; b = _b; }
	RIColor(U8 _r, U8 _g, U8 _b) { a = 255; r = _r; g = _g; b = _b; }
