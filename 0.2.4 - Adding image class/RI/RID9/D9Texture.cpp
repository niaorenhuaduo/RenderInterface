...
bool D9ImageTexture::OnRestore( LPDIRECT3DDEVICE9 pDevice )
{
	...
	if(FAILED(pDevice->CreateTexture(mTempImage.GetWidth(), mTempImage.GetHeight(), 0, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &mpTexture, NULL))) {
		LogE("D9ImageTexture: Failed to create the Texture.\n");
		mpTexture = NULL;
		return false;
	}
	mpTexture->GenerateMipSubLevels();		// Generate the mipmaps
