...
public:
	HRESULT VOnInitialize() {
		...
		mpRotatingShape->SetOrigin(rotatingTextRect.width / 2.0f, rotatingTextRect.height / 2.0f);
		mpRotatingShape->SetPosition(3.0f/4.0f * ri::VIRTUALXRESOLUTION, 3.0f / 4.0f * ri::VIRTUALYRESOLUTION);
	...
	void VOnUpdate() {
		...
		mpRotatingShape->Rotate(cosf(colorIndex * 2.0f) * 45.0f * mfElapsedTime);

		ri::FloatRect rotatingTextRect = mpRotatingText->GetLocalBoundingBox();
		mpRotatingShape->SetSize(rotatingTextRect.width, rotatingTextRect.height);
		mpRotatingShape->SetPosition(mpRotatingText->GetPosition() + ri::Vec2(rotatingTextRect.left, rotatingTextRect.top));
