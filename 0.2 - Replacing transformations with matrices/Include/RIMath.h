...
inline float RIDegreeToRadians(float deg) { return ((float)M_PI / 180.0f) * deg; }
inline float RIRadiansToDegree(float rad) { return (180.0f / (float)M_PI) * rad; }
...
public:
	...
	unsigned int	Random( unsigned int n );	// Returns a number from 0 to n (excluding n)
	float			Random( );					
...
inline unsigned int RIRandom::Random( unsigned int n )
{
	unsigned long y;
...
inline void RIRandom::SetRandomSeed(unsigned int n)
{
...
inline unsigned int RIRandom::GetRandomSeed(void)
{
	return(rseed);
}

inline void RIRandom::Randomize(void)
{
